...
def add_to_history(entry: dict):
    ...
    record = {
        ...
        "created_at": entry.get(
            "created_at",
            entry.get("time", entry.get("timestamp", datetime.now().strftime("%Y-%m-%d %H:%M")))
        ),
...
def compare_hooks(history: list) -> dict:
    ...
    return dict(sorted(result.items(), key=lambda x: x[1]["avg_score"], reverse=True))
...
def detect_winners(history: list, min_posts: int = 3) -> dict:
    ...
    winning_hooks = [h for h, d in hook_perf.items() if d["posts"] >= min_posts and d["avg_score"] >= 10][:3]
    weak_hooks = [h for h, d in hook_perf.items() if d["posts"] >= min_posts and d["avg_score"] < 5][:3]
    winning_niches = [n for n, d in niche_perf.items() if d["posts"] >= min_posts and d["avg_score"] >= 10][:2]
    best_slot = max(slot_perf, key=lambda x: slot_perf[x]["avg_score"], default="morning")
...
def posts_this_week(history: list) -> int:
    ...
    return sum(1 for p in history if p.get("time", "")[:10] >= week_ago)
