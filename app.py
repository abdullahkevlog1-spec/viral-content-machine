        ...
        def render_variation_tab(tab_obj, var_key, css_class, label):
            with tab_obj:
                ...
                if pending:
                    ...
                    if p_img:
                        img_url = generate_image_url(p_niche, p_text)
                        st.markdown("**🖼️ Image Preview — approve karey ya sirf text post karo:**")
                        st.image(img_url, use_container_width=True)
                        st.caption("Auto-generated by Pollinations.ai · Free · No API key")
                        col_a, col_b, col_c = st.columns([2, 2, 1])
                        with col_a:
                            do_img_post  = st.button("📸 Post WITH Image", key=f"do_img_{var_key}", type="primary", use_container_width=True)
                        with col_b:
                    ...
                    if do_img_post:
                        with st.spinner("📸 Image Facebook pe upload ho rahi hai..."):
                            fb_result = post_image_to_facebook(p_page_id, p_token, img_url, p_text)
